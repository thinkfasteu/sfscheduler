...
async function bundle({watchMode=false}){
  ...
  try {
    if (existsSync('index.html')){
      ...
      // Get base URL from environment, default to './' for local development
      const baseUrl = (envVars.BASE_URL || './').replace(/\/$/, '');
      const distPath = baseUrl === './' ? './dist/' : `${baseUrl}/dist/`;
      ...
      if (sriApp) block.push(`<link rel="modulepreload" href="${distPath}${manifest.app}" integrity="${sriApp}" crossorigin>`);
      (manifest.chunks||[]).forEach(c=>{
        const sri = manifest.integrities?.[c];
        if (sri) block.push(`<link rel="modulepreload" href="${distPath}${c}" integrity="${sri}" crossorigin>`);
      });
      ...
      // Then add the correct one for current base path
      html = html.replace(/(\s*)<script type="module" src="\.\/boot\.js"><\/script>/, `$1<script src="${distPath}runtime-config.js" data-generated="true"></script>\n$1<script type="module" src="./boot.js"></script>`);
      ...
      const escapedDistPath = escapeRegex(distPath);
      html = html.replace(new RegExp(`<link rel="modulepreload" href="${escapedDistPath}app\\.(?!${manifest.app.split('.')[1]})[a-f0-9]{10}\\.js[^>]*>\\n?`, 'g'),'');
      ...
      html = html
        // remove any modulepreload lines to OLD ./dist/* paths (not our new base-path aware ones)
        ...
        .replace(new RegExp(`<link[^>]+rel="modulepreload"[^>]+href="(?!${escapeRegex(distPath)})[^"]*dist/[^"]+"[^>]*>\\s*`, 'g'), '');
