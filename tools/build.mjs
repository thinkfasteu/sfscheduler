...
async function bundle({watchMode=false}){
  ...
  try {
    if (existsSync('index.html')){
      ...
      html = html
        // remove any modulepreload lines to our own ./dist/* or /dist/* files
        .replace(/<link[^>]+rel="modulepreload"[^>]+href="(?:\.\/|\/)?.?dist\/[^"]+"[^>]*>\s*/g, '')
        // drop integrity/crossorigin on internal /dist/* assets
        .replace(/(\s)integrity="[^"]*"/g, '')
        .replace(/(\s)crossorigin="[^"]*"/g, '')
        // remove the entire autogenerated preload comment block if present
        .replace(/<!-- AUTOGENERATED PRELOAD START -->[\s\S]*?<!-- AUTOGENERATED PRELOAD END -->/g, '');
      // -----------------------------------------------------------------------
